...
)
{
	...
	if (GetAsyncKeyState(VK_F10) & 1)
	{
		...
		if (item_name_to_id_map.count(user_input_string.AsString().data()) > 0)
		{
			...
			int index = item_name_to_id_map[user_input_string.AsString().data()];
			if (recipe_unlocks[index].m_Real == 0.0)
			{
				recipe_unlocks[index] = 1.0; // This value is ultimately what unlocks the recipe.
				...
				RValue* in = new RValue(static_cast<double>(index));
				RValue out;
				...
				delete in;
				g_ModuleInterface->Print(CM_LIGHTGREEN, "[Cookbook %s] - Unlocked recipe: %s", VERSION, user_input_string.AsString().data());
			}
			else {
				g_ModuleInterface->Print(CM_LIGHTYELLOW, "[Cookbook %s] - Ignoring already known recipe: %s", VERSION, user_input_string.AsString().data());
			}
		}
		else {
			g_ModuleInterface->Print(CM_LIGHTYELLOW, "[Cookbook %s] - Ignoring invalid recipe: %s", VERSION, user_input_string.AsString().data());
		}
	}
...
)
{
	if (load_items)
	{
			...
			if (name_key.m_Kind != VALUE_NULL && name_key.m_Kind != VALUE_UNDEFINED && name_key.m_Kind != VALUE_UNSET)
			{
				if (strstr(name_key.AsString().data(), "cooked_dishes"))
				{
					...
					if (recipe.m_Kind != VALUE_NULL && recipe.m_Kind != VALUE_UNDEFINED && recipe.m_Kind != VALUE_UNSET)
					{
						...
						{
							item_name_to_id_map[recipe_key.AsString().data()] = item_id.m_i64;
						}
					}
				}
			}
		...
		{
			g_ModuleInterface->Print(CM_LIGHTGREEN, "[Cookbook %s] - Loaded data for %d recipes!", VERSION, item_name_to_id_map.size());
		}
		else {
			g_ModuleInterface->Print(CM_LIGHTRED, "[Cookbook %s] - Failed to load data for recipes!", VERSION);
		}
...
void CreateHookGmlScriptTryLocationIdToString(AurieStatus& status)
{
	...
	{
		g_ModuleInterface->Print(CM_LIGHTRED, "[Cookbook %s] - Failed to get script (gml_Script_try_location_id_to_string)!", VERSION);
	}
	...
	if (!AurieSuccess(status))
	{
		g_ModuleInterface->Print(CM_LIGHTRED, "[Cookbook %s] - Failed to hook script (gml_Script_try_location_id_to_string)!", VERSION);
	}
}
...
void CreateHookGmlScriptSetupMainScreen(AurieStatus& status)
{
	...
	if (!AurieSuccess(status))
	{
		g_ModuleInterface->Print(CM_LIGHTRED, "[Cookbook %s] - Failed to get script (gml_Script_setup_main_screen@TitleMenu@TitleMenu)!", VERSION);
	}
	...
	if (!AurieSuccess(status))
	{
		g_ModuleInterface->Print(CM_LIGHTRED, "[Cookbook %s] - Failed to hook script (gml_Script_setup_main_screen@TitleMenu@TitleMenu)!", VERSION);
	}
}
...
EXPORTED AurieStatus ModuleInitialize(IN AurieModule* Module, IN const fs::path& ModulePath) {
	...
	g_ModuleInterface->Print(CM_LIGHTAQUA, "[Cookbook %s] - Plugin starting...", VERSION);
	
	CreateHookGmlScriptTryLocationIdToString(status);
	if (!AurieSuccess(status))
	{
		g_ModuleInterface->Print(CM_LIGHTRED, "[Cookbook %s] - Exiting due to failure on start!", VERSION);
		return status;
	}
	...
	g_ModuleInterface->Print(CM_LIGHTGREEN, "[Cookbook %s] - Plugin started!", VERSION);
