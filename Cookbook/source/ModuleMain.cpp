...
void UnlockRecipe(int item_id, CInstance* Self, CInstance* Other, bool silent)
{
	...
	if (!silent)
	{
		if (new_recipe_unlocked)
		{
			...
			RValue result;
			RValue arg0 = static_cast<double>(item_id);
...
)
{
	if (unlock_everything && unlock_recipes)
	{
		...
		{
			UnlockRecipe(pair.second, Self, Other, true);
		}
		g_ModuleInterface->Print(CM_LIGHTGREEN, "[%s %s] - Unlocked all cooking recipes!", MOD_NAME, VERSION);
	}
