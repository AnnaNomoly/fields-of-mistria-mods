...
void LoadFurnitureData()
{
	...
	{
		g_ModuleInterface->Print(CM_LIGHTGREEN, "[%s %s] - Loaded data for %d furniture recipes!", MOD_NAME, VERSION, item_name_to_id_map.size());
	}
	else {
		g_ModuleInterface->Print(CM_LIGHTRED, "[%s %s] - Failed to load data for furniture recipes!", MOD_NAME, VERSION);
	}
}
...
void CreateOrLoadConfigFile()
{
	...
	try
	{
		...
		// Try to find the mod_data/DIY directory.
		std::string diy_folder = mod_data_folder + "\\DIY";
		if (!std::filesystem::exists(diy_folder))
		{
			g_ModuleInterface->Print(CM_LIGHTYELLOW, "[%s %s] - The \"DIY\" directory was not found. Creating directory: %s", MOD_NAME, VERSION, diy_folder.c_str());
...
void UnlockFurnitureRecipe(std::string item_name, CInstance* Self, CInstance* Other, bool silent)
{
	...
	if (!silent)
	{
		if (new_furniture_unlocked)
		{
			...
			RValue result;
			RValue arg0 = static_cast<double>(furniture_ids[0]); // Can just use the first furniture ID associated with the internal name
...
)
{
	...
	if (unlock_everything && unlock_recipes)
	{
		unlock_recipes = false;
		for (auto& pair : item_name_to_id_map)
		{
			UnlockFurnitureRecipe(pair.first, Self, Other, true);
		}
		g_ModuleInterface->Print(CM_LIGHTGREEN, "[%s %s] - Unlocked all furniture recipes!", MOD_NAME, VERSION);
	}
