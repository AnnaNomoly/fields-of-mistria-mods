# D.I.Y.
A YYTK plugin for FieldsOfMistria that allows you to **learn any furniture recipe** in the game. This works on __existing__ saves!

## Instructions
1. Press `F8` to open the input dialog where you can specify one of the **furniture recipe names** from the table below.

> **NOTE**: The recipe name must be formatted *exactly* as shown in the table below!

2. The mod will output some information to the YYTK console window. If the recipe was **INVALID** you will get a warning message. If the recipe is **ALREADY KNOWN** you will get a different warning message. If the recipe is **NEW** you will get a success message, learn the recipe, and additionally the "new recipe" window will appear in the game!

## Furniture Recipe Table (FoM v0.13.2)
| Recipe Name |
| ----------- |
| alda_pot_replica |
| bakery_bread_basket |
| bakery_cake |
| bakery_cake_case |
| bakery_cake_slice |
| bakery_cookie_jar |
| bakery_cutting_board |
| basic_area_rug |
| basic_bed |
| basic_bed_double |
| basic_bookshelf |
| basic_chair |
| basic_doormat |
| basic_dresser |
| basic_flooring |
| basic_flower_pot |
| basic_nightstand |
| basic_oil_lamp |
| basic_table |
| basic_wall_window |
| basic_wallpaper |
| basic_wood_chest |
| bathroom_bench |
| bathroom_curtain_stand |
| bathroom_floor_mirror |
| bathroom_sink |
| bathroom_tile_wall |
| bathroom_wall_mirror |
| bathroom_wall_sconce |
| bathroom_wall_towel |
| bathtub |
| big_bell |
| bumblebee_plushie |
| butterfly_bed |
| butterfly_double_bed |
| butterfly_wall_lamp |
| cabin_bed |
| cabin_bed_double |
| cabin_bookshelf |
| cabin_chair |
| cabin_dresser |
| cabin_firewood |
| cabin_flooring |
| cabin_lamp |
| cabin_nightstand |
| cabin_stove |
| cabin_table |
| cabin_wall_shelf |
| cabin_wall_window |
| cabin_wallpaper |
| campfire |
| caterpillar_chair |
| cavern_crystal_lamp |
| cavern_emerald_rock |
| cavern_floor_brazier |
| cavern_hewn_rock_bookshelf |
| cavern_hewn_rock_table |
| cavern_mossy_rug |
| cavern_mushroom_stool |
| cavern_rock_flooring |
| cavern_rock_wallpaper |
| cavern_silver_node |
| cavern_stone_arch |
| cavern_wall_brazier |
| chess_bishop |
| chess_king |
| chess_knight |
| chess_pawn |
| chess_queen |
| chess_rook |
| chess_table |
| coffee_mug |
| color_paving_stone |
| coral_chair |
| coral_lamp |
| coral_stone_archway |
| coral_storage_chest |
| coral_table |
| cottage_bed |
| cottage_bed_double |
| cottage_chair |
| cottage_dresser |
| cottage_flooring |
| cottage_garden_arch |
| cottage_garden_planter |
| cottage_garden_shed |
| cottage_garden_trellis |
| cottage_garden_workbench |
| cottage_nightstand |
| cottage_plant_shelf |
| cottage_potted_fig |
| cottage_rug |
| cottage_table |
| cottage_wall_flowers |
| cottage_wall_window |
| cottage_wallpaper |
| counter_basic_v1 |
| counter_basic_v2 |
| counter_cabin_cherry |
| counter_cabin_oak |
| counter_cabin_walnut |
| counter_cake_chocolate |
| counter_cake_double_chocolate |
| counter_cake_strawberry |
| counter_cottage_v1 |
| counter_cottage_v2 |
| crop_faux_ash_mushroom |
| crop_faux_basil |
| crop_faux_beet |
| crop_faux_blue_conch_shell |
| crop_faux_breath_of_fire |
| crop_faux_broccoli |
| crop_faux_burdock_root |
| crop_faux_cabbage |
| crop_faux_carrot |
| crop_faux_catmint |
| crop_faux_cauliflower |
| crop_faux_cave_kelp |
| crop_faux_celosia |
| crop_faux_chestnut |
| crop_faux_chickpea |
| crop_faux_chili_pepper |
| crop_faux_chrysanthemum |
| crop_faux_corn |
| crop_faux_cosmos |
| crop_faux_cranberry |
| crop_faux_crocus |
| crop_faux_crystal_berries |
| crop_faux_crystal_rose |
| crop_faux_cucumber |
| crop_faux_daffodil |
| crop_faux_daikon_radish |
| crop_faux_daisy |
| crop_faux_dandelion |
| crop_faux_dill |
| crop_faux_earthshroom |
| crop_faux_fennel |
| crop_faux_fiddlehead |
| crop_faux_flame_pepper |
| crop_faux_fog_orchid |
| crop_faux_frost_lily |
| crop_faux_garlic |
| crop_faux_heather |
| crop_faux_holly |
| crop_faux_horseradish |
| crop_faux_hot_potato |
| crop_faux_ice_block |
| crop_faux_iris |
| crop_faux_jasmine |
| crop_faux_lava_chestnuts |
| crop_faux_lilac |
| crop_faux_marigold |
| crop_faux_middlemist |
| crop_faux_mines_mussels |
| crop_faux_moon_fruit |
| crop_faux_morel_mushroom |
| crop_faux_narrows_moss |
| crop_faux_nettle |
| crop_faux_night_queen |
| crop_faux_onion |
| crop_faux_oregano |
| crop_faux_oyster_mushroom |
| crop_faux_peas |
| crop_faux_pineshroom |
| crop_faux_pink_scallop_shell |
| crop_faux_poinsettia |
| crop_faux_potato |
| crop_faux_pumpkin |
| crop_faux_rice |
| crop_faux_rockroot |
| crop_faux_rosemary |
| crop_faux_sage |
| crop_faux_sand_dollar |
| crop_faux_sea_grapes |
| crop_faux_sesame |
| crop_faux_shadow_flower |
| crop_faux_shale_grass |
| crop_faux_snapdragon |
| crop_faux_snow_peas |
| crop_faux_snowdrop_anemone |
| crop_faux_spirit_mushroom |
| crop_faux_spirula_shell |
| crop_faux_strawberry |
| crop_faux_sugar_cane |
| crop_faux_sunflower |
| crop_faux_sweet_potato |
| crop_faux_sweetroot |
| crop_faux_tea |
| crop_faux_temple_flower |
| crop_faux_thorn_vine |
| crop_faux_thyme |
| crop_faux_tide_lettuce |
| crop_faux_tomato |
| crop_faux_tulip |
| crop_faux_turnip |
| crop_faux_upper_mines_mushroom |
| crop_faux_viola |
| crop_faux_walnut |
| crop_faux_watermelon |
| crop_faux_wheat |
| crop_faux_wild_leek |
| crop_sign_apple |
| crop_sign_basil |
| crop_sign_beet |
| crop_sign_broccoli |
| crop_sign_burdock_root |
| crop_sign_cabbage |
| crop_sign_carrot |
| crop_sign_catmint |
| crop_sign_cauliflower |
| crop_sign_celosia |
| crop_sign_cherry |
| crop_sign_chickpea |
| crop_sign_chili_pepper |
| crop_sign_chrysanthemum |
| crop_sign_coconut |
| crop_sign_corn |
| crop_sign_cosmos |
| crop_sign_cranberry |
| crop_sign_cucumber |
| crop_sign_daffodil |
| crop_sign_daikon_radish |
| crop_sign_daisy |
| crop_sign_dill |
| crop_sign_frost_lily |
| crop_sign_garlic |
| crop_sign_heather |
| crop_sign_iris |
| crop_sign_jasmine |
| crop_sign_lemon |
| crop_sign_lilac |
| crop_sign_marigold |
| crop_sign_moon_fruit |
| crop_sign_night_queen |
| crop_sign_onion |
| crop_sign_orange |
| crop_sign_oregano |
| crop_sign_peach |
| crop_sign_pear |
| crop_sign_peas |
| crop_sign_poinsettia |
| crop_sign_pomegranate |
| crop_sign_potato |
| crop_sign_pumpkin |
| crop_sign_rice |
| crop_sign_rosemary |
| crop_sign_sage |
| crop_sign_snapdragon |
| crop_sign_snow_peas |
| crop_sign_snowdrop_anemone |
| crop_sign_strawberry |
| crop_sign_sugar_cane |
| crop_sign_sunflower |
| crop_sign_sweet_potato |
| crop_sign_tea |
| crop_sign_temple_flower |
| crop_sign_thyme |
| crop_sign_tomato |
| crop_sign_tulip |
| crop_sign_turnip |
| crop_sign_viola |
| crop_sign_watermelon |
| crop_sign_wheat |
| crystal_string_lights |
| dirt_clear |
| dirt_dot_one |
| dirt_dot_three |
| dirt_dot_two |
| dragon_altar |
| dragon_priestess_fountain |
| dragon_statue_replica |
| espresso_cup |
| espresso_machine |
| explorer_bed |
| explorer_chair |
| explorer_double_bed |
| explorer_flooring |
| explorer_globe |
| explorer_lamp |
| explorer_rug |
| explorer_trunk_table |
| explorer_wallpaper |
| fall_double_bed |
| fall_single_bed |
| fall_table |
| farm_bridge |
| farm_house_calendar |
| field_clear |
| field_flower |
| field_flower_two |
| field_grass |
| field_grass_two |
| firefly_wall_string_lights |
| fish_plush_toy |
| fishing_bed |
| fishing_chair |
| fishing_double_bed |
| fishing_flooring |
| fishing_lamp |
| fishing_oar_wall_decor |
| fishing_rug |
| fishing_table |
| fishing_tank |
| fishing_trophy_shelf |
| fishing_wall_plaque |
| fishing_wallpaper |
| fishing_window |
| gold_floor_mirror |
| gold_wall_mirror |
| gryphon_statue_replica |
| haunted_attic_armoire |
| haunted_attic_bed |
| haunted_attic_candle_cluster |
| haunted_attic_candle_single |
| haunted_attic_chair |
| haunted_attic_double_bed |
| haunted_attic_dress_form |
| haunted_attic_flooring |
| haunted_attic_nightstand |
| haunted_attic_rocking_chair |
| haunted_attic_table |
| haunted_attic_wall_cobweb_left |
| haunted_attic_wall_cobweb_right |
| haunted_attic_wall_shelf |
| haunted_attic_wall_window |
| haunted_attic_wallpaper |
| herringbone_tile_floor |
| insect_flooring |
| insect_rug |
| insect_wallpaper |
| journal |
| ladybug_table |
| large_fluffy_rug |
| large_paving_stone |
| lava_caves_bed |
| lava_caves_chair |
| lava_caves_chest |
| lava_caves_diamond_rock |
| lava_caves_fence |
| lava_caves_floor_lamp |
| lava_caves_flooring |
| lava_caves_gold_node |
| lava_caves_stone_arch |
| lava_caves_table |
| lava_caves_wallpaper |
| low_tide_flooring |
| mermaid_bed |
| minecart |
| miners_copper_node |
| miners_crate_chair |
| miners_crate_chest |
| miners_floor_lamp |
| miners_flooring |
| miners_ruby_rock |
| miners_shelves |
| miners_wall_lamp |
| miners_wallpaper |
| mines_wooden_arch |
| mistria_history_book |
| mistria_history_scroll |
| mistria_wall_map |
| moon_gate |
| moth_lamp |
| ocarina_sprite_statue |
| octopus_plushie |
| ornamental_iron_node |
| ornamental_sapphire_rock |
| ornate_flagstone |
| ornate_rug_large_rectangle |
| ornate_rug_large_round |
| ornate_rug_large_square |
| ornate_rug_medium_round |
| ornate_rug_medium_square |
| ornate_rug_small |
| picket_fence |
| picnic_basket |
| picnic_donut_plate |
| picnic_hamburger_plate |
| picnic_pie_plate |
| picnic_place_setting |
| picnic_rug |
| picnic_sandwich_plate |
| picnic_sunflower_vase |
| round_cushioned_stool |
| sand_rug |
| small_bathmat |
| spooky_haybale |
| spring_bed |
| spring_double_bed |
| spring_flooring |
| spring_lamp |
| spring_potted_tree |
| spring_rug |
| spring_sofa |
| spring_wallpaper |
| square_tile_floor |
| star_cushion |
| star_lantern |
| star_viewing_blanket |
| starry_flooring |
| starter_bird_house_red |
| starter_potted_plant |
| starter_scarecrow |
| starter_shipping_box |
| starter_stone_path |
| starter_stone_path_double |
| starter_well |
| starter_wheelbarrow |
| starter_wood_fence |
| stone_bench |
| stone_dragon_figure |
| stone_lamp |
| stone_storage_chest |
| stone_table |
| stone_wall |
| summer_bed |
| summer_chair |
| summer_double_bed |
| summer_flooring |
| summer_jar_lamp |
| summer_rug |
| summer_table |
| summer_wallpaper |
| summit_wallpaper |
| tide_touched_cavern_wall |
| tile_roof_fence |
| toilet |
| training_dummy |
| water_sprite |
| witch_queen_bed |
| witch_queen_cauldron |
| witch_queen_chair |
| witch_queen_double_bed |
| witch_queen_dressing_table |
| witch_queen_flooring |
| witch_queen_moon_lamp |
| witch_queen_nightstand |
| witch_queen_pillar |
| witch_queen_rug |
| witch_queen_table |
| witch_queen_throne |
| witch_queen_wallpaper |
| worn_bed |
| worn_chair |
| worn_fireplace |
| worn_flooring |
| worn_garden_lamp |
| worn_nightstand |
| worn_rug |
| worn_table |
| worn_wallpaper |
| worn_well |
| worn_window |
